...
void showmap(giveplayer &tmp){
    ...
    for (int i=0;i<tmp.mpsize;i++){
        for (int j=0;j<tmp.mpsize;j++)
            if (tmp.smap[i][j]) fprintf(fp,"%d",tmp.smap[i][j]);
            else fprintf(fp," ");
        fprintf(fp,"\n");
    }
    fprintf(fp,"\n");
...
void randomfood(int x,giveplayer &tmp){
    for (int i=0;i<tmp.mpsize;i++)
        for (int j=0;j<tmp.mpsize;j++)
            if (tmp.smap[i][j]==2) tmp.smap[i][j]=0;
...
void fillall(int x,giveplayer &tmp){
    for (int i=0;i<tmp.mpsize;i++){
        for (int j=0;j<tmp.mpsize;j++){
...
void getcmd(int *a,giveplayer &tmp){
    a[0]=tmp.food.x;
    a[1]=tmp.food.y;
    a[4]=tmp.mpsize;
    for (int i=0;i<40;i++)
...
int main(int argv,char * argc[])
{
    ...
    if (!RESULTA) RESULTA=tmp.p1.bodylen;
    if (!RESULTB) RESULTB=tmp.p2.bodylen;
